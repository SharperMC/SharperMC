	...
	public class ClientListener
	{
		...
		public void StartListening()
		{
			...
			while (_serverListener.Server.IsBound)
			{
				try
				{
					if (_disposed)
					{
						Console.WriteLine("Ejecting from ClientListener loop.");
						break;
					}
					if (!_serverListener.Pending()) continue;
					TcpClient client = _serverListener.AcceptTcpClient();
					Console.WriteLine("[{0}] Accepting new TCP client connection.", GetType().Name);
		...
		private void HandleClient(TcpClient client)
		{
			...
			while (clientWrapper.Connected)
			{
				...
				{
					if(SharperMC.Instance.Server.ServerSettings.ReportFatalExceptions)
